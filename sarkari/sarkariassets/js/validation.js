var base_url = 'http://cricnotify.com/';
...
function alphanum_validate(val, minLen, maxLen, fName, errId) {
	...
	val = val.toString();
	if (val == '') {
		errs = 'Please enter ' + fName;
	} else if (val.length < minLen) {
		errs = 'Please enter minimum ' + minLen + ' characters';
	} else if (maxLen > 0 && val.length > maxLen) {
		errs = 'Please enter ' + maxLen + ' characters';
	} else if (!alphanum.test(val)) {
		errs = 'Please enter Alphanumeric only';
	} else {
		errs = '';
	}
	return errs;
...
function string_validate(val, minLen, maxLen, fName, errId) {
	...
	if (val == '') {
		...
		errs = 'Please enter ' + maxLen + ' characters';
	} else if (!charFilter.test(val)) {
		errs = 'Please enter alphabets only';
	} else {
		errs = '';
	}
	return errs;
...
function varchar_validate_dd(val, minLen, maxLen, fName, errId) {
	...
	val = val.toString();
	if (val == '') {
		errs = 'Please select ' + fName;
	} else if (val.length < minLen) {
		errs = 'Please select minimum ' + minLen + ' characters';
	} else if (maxLen > 0 && val.length > maxLen) {
		errs = 'Please select ' + maxLen + ' characters';
	} else {
		errs = '';
	}
	return errs;
...
function varchar_validate(val, minLen, maxLen, fName, errId) {
	if (typeof errId === "undefined" || errId === null) {
		errId = 'enter';
	} else if (errId == 'select') {
		errId = 'select';
	} else {
		errId = 'enter';
	}
	...
	val = val.toString();
	if (val == '') {
		errs = 'Please ' + errId + ' ' + fName;
	} else if (val.length < minLen) {
		errs = 'Please ' + errId + ' minimum ' + minLen + ' characters';
	} else if (maxLen > 0 && val.length > maxLen) {
		errs = 'Please ' + errId + ' ' + maxLen + ' characters';
	} else {
		errs = '';
	}
	return errs;
...
function int_validate(val, minLen, maxLen, fName, errId) {
	...
	if (val == '') {
		...
		errs = 'Please enter minimum ' + minLen + ' digits';
	} else if (maxLen > 0 && val.length > maxLen) {
		errs = 'Please enter ' + maxLen + ' digits';
	}
	//else if(isNaN(val)){errs = 'Please enter numbers only';}
	else if (!number_f.test(val)) {
		errs = 'Please enter numbers only';
	} else {
		errs = '';
	}
	return errs;
